...
def getReportBetween2Tokens6(request):
    ...
    if request.method == 'POST':
        ...
        for ind,i in enumerate(datatoken):
            ...
            token6 = i.followWord if i.followWord != ' ' else '[SPACE]'
...
def getReportBetween2Tokens5(request):
    ...
    if request.method == 'POST':
        ...
        for ind,i in enumerate(datatoken):
            ...
            token3 = i.token3 if i.token3 != ' ' else '[SPACE]'
            token4 = i.token4 if i.token4 != ' ' else '[SPACE]'
            token5 = i.followWord if i.followWord != ' ' else '[SPACE]'
            mergeToken2 = i.token1 + i.token2
            mergeToken3 = i.token1 + i.token2 + i.token3
            mergeToken4 = i.token1 + i.token2 + i.token3 + i.token4
            result['data'].append({
...
def getReportBetween2Tokens4(request):
    ...
    if request.method == 'POST':
        ...
        for ind,i in enumerate(datatoken):
            ...
            token2 = i.token2 if i.token2 != ' ' else '[SPACE]'
            token3 = i.token3 if i.token3 != ' ' else '[SPACE]'
            token4 = i.followWord if i.followWord != ' ' else '[SPACE]'
            mergeToken2 = i.token1 + i.token2
            mergeToken3 = i.token1 + i.token2 + i.token3
            result['data'].append({
                'No': '<div >' + str(number) + '</div>',
...
@csrf_exempt
def getReportTextByMergeTokenExpression(request):
    if request.method == 'POST':
        #取得資料
        result = {'status':'1'} #預設沒找到
        ...
        try:
            ...
            query += f'COUNT(*) AS times, '
            for i in range(len(tokenList)):
                if i != (len(tokenList)-1):
                    query += f'b{i}.token + '
                else:
                    query += f'b{i}.token as mergeToken, a0.reportID, t.reportText from [textToken] as a0 '
            for j in range(1,len(tokenList)):
                query += f'inner join [textToken] as a{j} on a{j-1}.reportID=a{j}.reportID and (a{j-1}.posEnd+1)=a{j}.posStart AND a{j-1}.posStart>0 AND a{j}.posStart>0 '
            for k in range(len(tokenList)):
                query += f'INNER JOIN Vocabulary AS b{k} ON a{k}.tokenID = b{k}.tokenID '
            query += f'inner join analyseText as t on a0.reportID = t.reportID GROUP BY '
            for q in range(len(tokenList)):
                query += f'b{q}.token, '
            query += f' a0.reportID, t.reportText'
            query += f") as a left join [Vocabulary] as b on a.mergeToken=b.token where a.mergeToken = '{mergetoken}' order by times desc;"
                    # # # # print(type(words))
                    # # # # print(mergetoken)
...
@csrf_exempt
def getReportByReportID(request):
    if request.method == 'POST':
        #取得資料
        result = {'status':'1'} #預設沒找到
        ...
        try:
            ...
            if reportID == "":
                raise Exception("reportID錯誤")

            query = ''' select reportText from analyseText where reportID = ?'''
            args = [reportID]
            ...
            result['data'].append(report.reportText)
            print(result['data'])
